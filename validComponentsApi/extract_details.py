...
def extract_info_from_gpu(title: str) -> str:
...
def extract_brand_from_ssd(title: str) -> dict:
    ...
    for media in media_types:
        title = title.replace(media.lower(), "")

    title = re.sub(r"\b\d+(\.\d+)?\s?(gb|tb)\b", "", title)
    return {
        "brand": brand,
...
def extract_brand_from_cpu_cooler(title: str) -> dict:
    ...
    for brand in CPU_COOLER_BRANDS:
        title = title.replace(brand.lower(), "")
    return {
        "brand": brand,
        "model": re.sub(r"\s+", " ", title).strip(),
...
def extract_brand_from_power_supply(title: str) -> dict:
    ...
    for brand in POWER_SUPPLY_BRANDS:
        title = title.replace(brand, "")
    title = re.sub(r"\b\d{3,4}\s?w\b", "", title.lower())
...
def extract_brand_from_motherboard(title: str) -> dict:
    ...
    for brand in MOTHERBOARD_BRANDS:
        title = title.replace(brand, "")
    return {
        "brand": brand,
        "model": title.strip(),
